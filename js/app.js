...
async function mostrarApp() {
    ...
    // Aplica el rol según el staff logueado (Administrador ve todo; Entrenador
    // solo Horarios + ingreso). El entrenador no puede cambiar de rol.
    try {
        const s = await Api.getSession();
        const rol = (s && s.user && /entren/i.test(s.user.rol || '')) ? 'entrenador' : 'admin';
        const sel = document.getElementById('rol-demo');
        if (sel) sel.value = rol;
        aplicarRolDemo(rol);
        const verComo = document.getElementById('ver-como-wrap');
        if (verComo) verComo.style.display = (rol === 'entrenador') ? 'none' : '';
    } catch (_) { }
}
