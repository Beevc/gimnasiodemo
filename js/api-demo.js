...
const Api = (() => {
    ...
    return {
        ...
        async login(email, password) {
            // Valida contra el staff (correo + clave). El rol define qué ve en el panel.
            const e = String(email || '').trim().toLowerCase();
            const staff = (typeof STAFF_DEMO !== 'undefined' ? STAFF_DEMO : [])
                .find(s => String(s.correo || '').trim().toLowerCase() === e && s.clave === password);
            if (!staff) throw new Error('Correo o contraseña incorrectos');
            const sesion = { user: { id: staff.id, email: staff.correo, nombre: staff.nombre, rol: staff.rol } };
            try { localStorage.setItem('gd_sesion_v1', JSON.stringify(sesion)); } catch (_) { }
            return sesion;
        },

        async logout() {
            try { localStorage.removeItem('gd_sesion_v1'); } catch (_) { }
            location.reload();
        },

        async getSession() {
            // Sin sesión guardada => app.js muestra la pantalla de login.
            try { const r = localStorage.getItem('gd_sesion_v1'); if (r) return JSON.parse(r); } catch (_) { }
            return null;
        },
