...
function renderReservar() {
    ...
    // Feriado: el gimnasio está cerrado hoy → no se reserva.
    const fer = (typeof Horarios.feriadoHoy === 'function') ? Horarios.feriadoHoy() : null;
    if (fer) {
        cont.innerHTML = `
            <h2 class="text-xl font-black text-white">Reservar horario</h2>
            <div class="bg-amber-500/10 border border-amber-500/30 rounded-2xl p-5 text-center space-y-2">
                <i class="fa-solid fa-calendar-xmark text-amber-400 text-2xl"></i>
                <p class="text-slate-100 font-black">Hoy el gimnasio está cerrado</p>
                <p class="text-sm text-amber-300 font-bold">${esc(fer.mensaje || 'Feriado')}</p>
                <p class="text-xs text-slate-400">No hay reservas por hoy. ¡Nos vemos el próximo día hábil!</p>
            </div>`;
        return;
    }
