...
const Acceso = (() => {
    ...
    const cancelarReserva = (cid) => {
        const store = _load();
        if (store[cid]) { delete store[cid]; _save(store); }
    };
    ...
    const reservasDelDia = (fechaISO) => {
        ...
        for (const cid of Object.keys(store)) {
            const r = store[cid];
            if (r && r.fecha === fechaISO) out.push({ clienteId: cid, bloque: r.bloque, qr: r.qr, usado: !!r.usado, generado: !!r.generado });
        }
        return out;
    };
    ...
    const validar = (payload) => {
        ...
        const store = _load();
        const r = store[d.cid];
        if (!r || r.fecha !== hoyISO()) return { ok: false, estado: 'DENEGADO', motivo: 'Sin reserva activa hoy', cliente: c };
        ...
        // Primer ingreso válido: se marca como usado (un solo ingreso por día).
